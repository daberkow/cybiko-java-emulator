...
class AppEntryTest {
    ...
    void extensionFromAppFile() {
        AppEntry entry = new AppEntry(Path.of("/apps/calc.app"), "calc.app", 1024, Instant.now());
        assertEquals("app", entry.extension());
    }

    @Test
    void extensionFromUpperCase() {
        AppEntry entry = new AppEntry(Path.of("/apps/Game.APP"), "Game.APP", 2048, Instant.now());
        assertEquals("app", entry.extension());
    }

    @Test
    void extensionEmptyForNoExtension() {
        AppEntry entry = new AppEntry(Path.of("/apps/README"), "README", 100, Instant.now());
        assertEquals("", entry.extension());
    }

    @Test
    void extensionEmptyForTrailingDot() {
        AppEntry entry = new AppEntry(Path.of("/apps/file."), "file.", 50, Instant.now());
        assertEquals("", entry.extension());
    }
    ...
    void lastModifiedDateTime() {
        ...
        AppEntry entry = new AppEntry(Path.of("/apps/test.app"), "test.app", 512, instant);
